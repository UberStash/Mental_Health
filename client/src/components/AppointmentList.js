   ...
   <List.Item>
    ...
    content={<Grid>
      ...
      <Header as='h3'>Dr, {user.first_name}, {user.last_name}</Header>
          ...
          <Grid.Row>
          <Header as='h3'><Icon name='info' />Start Time: {convertTime(user.appt_start.replace('T', " ").replace('Z', '')).slice(0, 16)}</Header>
          </Grid.Row>
         <Grid.Row>
         <Header as='h3'><Icon name='info' />End Time: {convertTime(user.appt_end.replace('T', " ").replace('Z', '')).slice(0, 16)}</Header>         </Grid.Row>
          <Grid.Row>
          ...
          <Grid.Row>
          <Header><Icon name='key'/>{user.appt_password}</Header>
          </Grid.Row>
          </Grid>
         }
    on='click'
    ...
    trigger={<Segment inverted >
      <Header textAlign='center' as='h1'>{convertTime(user.appt_start.replace('T', " ").replace('Z', '')).slice(0, 16)}</Header> 
      </Segment>}
  /> 
</List.Item>
));
